...
describe('ValleySnowLoadCalculator', () => {
  ...
  describe('Project Operations', () => {
    it('should create project successfully', async () => {
      ...
      expect(mockCheckpointMgr.markProjectActive).toHaveBeenCalledWith(projectId);
    });
    ...
    it('should load project with data integrity check', async () => {
      ...
      expect(mockCheckpointMgr.markProjectActive).toHaveBeenCalledWith('test-project');
    });
    ...
    it('should delete project safely', async () => {
      ...
      expect(mockCheckpointMgr.markProjectInactive).toHaveBeenCalledWith('test-project');
    });
  });
  ...
  describe('Recovery Operations', () => {
    ...
    it('should recover from checkpoint', async () => {
      ...
      mockCheckpointMgr.restoreFromCheckpoint.mockResolvedValue(mockRecoveredData);
  ...
  describe('Comprehensive Error Handling', () => {
    it('should handle database errors gracefully with recovery attempts', async () => {
      mockDb.loadProject.mockRejectedValue(new Error('Database connection failed'));

      // Should attempt recovery from checkpoint
      mockCheckpointMgr.getRecoveryOptions.mockResolvedValue([
        { type: 'checkpoint', id: 'checkpoint_1', timestamp: new Date(), operation: 'auto_save' }
      ]);

      await expect(
        calculator.loadProject('test-project')
    ...
    it('should handle timeout errors in operations', async () => {
      ...
      await expect(
        calculator.loadProject('timeout-test')
      ).rejects.toThrow();
    ...
    it('should maintain operation idempotency', async () => {
      ...
      mockDb.get.mockResolvedValue({ result: JSON.stringify({ id: 'cached-result' }) });
