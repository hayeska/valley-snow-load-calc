...
describe("Data Persistence Integration Tests", () => {
  ...
  beforeEach(async () => {
    ...
    testDbPath = path.join(
      tempDir,
      `integration_test_${Date.now()}_${Math.random()}.db`,
    );
  ...
  describe("Complete CRUD Workflow", () => {
    test("should complete full project lifecycle", async () => {
      ...
      const updatedProject = await calculator.updateGeometry(
        projectId,
        updatedGeometry,
      );
      ...
      const calculatedProject = await calculator.updateInputs(
        projectId,
        updatedInputs,
      );
      expect(calculatedProject.inputs.groundSnowLoad).toBe(45);
      expect(calculatedProject.results.balancedLoads.northRoof).toBeGreaterThan(
        0,
      );
  ...
  describe("Crash Simulation and Recovery", () => {
    ...
    test("should handle checkpoint recovery", async () => {
      ...
      const latestCheckpoint = recoveryOptions.find(
        (opt) => opt.type === "checkpoint",
      );
      if (latestCheckpoint) {
        const recovered = await calculator.recoverFromCheckpoint(
          latestCheckpoint.id,
        );
    ...
    test("should handle concurrent operations safely", async () => {
      ...
      const operations = Array(operationCount)
        ...
        .map(async (_, i) => {
          ...
          await calculator.createProject(
  ...
  describe("Error Scenarios and Edge Cases", () => {
    test("should handle invalid project IDs", async () => {
      const invalidIds = [
        ...
        undefined,
        "   ",
        "invalid-id-with-spaces and symbols!@#",
      ];

      for (const invalidId of invalidIds) {
        await expect(
          calculator.loadProject(invalidId as string),
        ).rejects.toThrow();
      }
    });
    ...
    test("should handle malformed geometry data", async () => {
      ...
      for (const invalidGeometry of invalidGeometries) {
        await expect(
          calculator.updateGeometry(projectId, invalidGeometry),
        ).rejects.toThrow();
      }
    ...
    test("should handle invalid input parameters", async () => {
      ...
      for (const invalidInput of invalidInputs) {
        await expect(
          calculator.updateInputs(projectId, invalidInput),
        ).rejects.toThrow();
      }
    });
    ...
    test("should handle database disk full scenarios", async () => {
      ...
      await expect(
        calculator.saveProject(await calculator.loadProject(projectId)!),
  ...
  describe("Performance and Load Testing", () => {
    test("should handle bulk project operations", async () => {
      ...
      const createPromises = Array(projectCount)
        .fill(null)
        .map((_, i) =>
    ...
    test("should maintain performance under sustained load", async () => {
      ...
      console.log(
        `Load test: ${fulfilled}/${operationCount} operations succeeded in ${duration}ms`,
      );
    });
    ...
    test("should handle memory pressure gracefully", async () => {
      ...
      // Save large projects
      const savePromises = largeProjects.map((project) =>
        db.saveProject(project),
      );
      const saveResults = await Promise.allSettled(savePromises);

      const saveSuccess = saveResults.filter(
        (r) => r.status === "fulfilled",
      ).length;
      expect(saveSuccess).toBe(largeProjectCount);

      // Load and verify large projects
      const loadPromises = largeProjects.map((project) =>
        calculator.loadProject(project.id),
      );
      const loadResults = await Promise.allSettled(loadPromises);

      const loadSuccess = loadResults.filter(
        (r) => r.status === "fulfilled",
      ).length;
