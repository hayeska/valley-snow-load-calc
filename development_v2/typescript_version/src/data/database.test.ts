...
describe("DatabaseManager - Comprehensive Data Persistence Tests", () => {
  ...
  beforeEach(async () => {
    ...
    testDbPath = path.join(
      tempDir,
      `test_valley_calc_${Date.now()}_${Math.random()}.db`,
    );
  ...
  describe("Project Load Operations", () => {
    ...
    test("should handle database connection failures during load", async () => {
      ...
      await expect(db.loadProject(projectId)).rejects.toThrow(
        "Connection lost",
      );
      expect(mockLogger.logError).toHaveBeenCalled();
  ...
  describe("Error Scenarios and Crash Simulations", () => {
    ...
    test("should handle database lock errors with retry", async () => {
      ...
      let callCount = 0;
      db["runWithRetry"] = jest
        .fn()
        .mockImplementation(async (sql, operation) => {
          callCount++;
          if (callCount === 1) {
    ...
    test("should handle transaction rollback on failure", async () => {
      ...
      await expect(
        db["executeInTransaction"](async (transaction) => {
          ...
          // This should work
          await db["run"](
            "INSERT INTO projects (id, name, data, checksum) VALUES (?, ?, ?, ?)",
    ...
    test("should handle partial write failures", async () => {
      ...
      db["run"] = jest.fn().mockImplementation(async (sql) => {
        callCount++;
        if (callCount === 2) {
          // Fail on second SQL call
          throw new Error("Partial write failure");
        }
        return originalRun(sql);
      });

      await expect(db.saveProject(testData)).rejects.toThrow(
  ...
  describe("Retry Logic and Data Loss Prevention", () => {
    ...
    test("should clean up expired idempotency keys", async () => {
      ...
      await db["run"](
        "INSERT INTO idempotency_keys (key, operation, expires_at, result) VALUES (?, ?, ?, ?)",
  ...
  describe("Recovery and Backup Operations", () => {
    ...
    test("should create and restore from checkpoints", async () => {
      ...
      const recovered = await db["restoreFromCheckpoint"](
    ...
    test("should provide recovery options", async () => {
      ...
      expect(recoveryOptions.some((opt) => opt.type === "checkpoint")).toBe(
        true,
      );
      expect(
        recoveryOptions.some((opt) => opt.type === "last_good_state"),
      ).toBe(true);
    });
  });
  ...
  describe("Performance and Load Testing", () => {
    ...
    test("should handle bulk operations efficiently", async () => {
      ...
      const bulkProjects = allProjects.filter((p) =>
        p.name.startsWith("Bulk Test"),
      );
    ...
    test("should maintain performance under concurrent load", async () => {
      ...
      console.log(
        `Concurrent load test: ${fulfilled}/${concurrentOperations} succeeded in ${duration}ms`,
      );
    });
  });
});
