...
export class DatabaseManager {
  ...
  private async createIndexesWithRecovery(): Promise<void> {
    const indexes = [
      ...
      },
      {
        ...
        sql: "CREATE INDEX IF NOT EXISTS idx_projects_updated ON projects (updated_at)",
      },
      {
        name: "idx_checkpoints_project",
        sql: "CREATE INDEX IF NOT EXISTS idx_checkpoints_project ON checkpoints (project_id)",
      },
      {
      ...
      },
      {
        ...
        sql: "CREATE INDEX IF NOT EXISTS idx_idempotency_expires ON idempotency_keys (expires_at)",
      },
    ];
    ...
    try {
      ...
      if (fs.existsSync(this.dbPath)) {
        try {
          ...
          this.logger.info("Database backup created for recovery", {
            backupPath,
          });
        } catch (backupError) {
          this.logger.logError(backupError as Error, {
            operation: "create_db_backup",
          });
        }
      }
      ...
      if (!this.validateChecksum(jsonData, checksum)) {
        ...
        if (recoveredData) {
          this.logger.logRecoveryAction(
            "Data recovered from checkpoint",
            true,
            { projectId },
          );
  ...
  async deleteProject(projectId: string): Promise<boolean> {
    try {
      ...
      await this.run("DELETE FROM checkpoints WHERE project_id = ?", [
        projectId,
      ]);

      // Delete project
      const result = await this.run("DELETE FROM projects WHERE id = ?", [
        projectId,
      ]);
      ...
      this.logger.logDatabaseOperation("delete_project", true, 0, {
        projectId,
      });
      return true;
    } catch (error) {
  ...
  private isRecoverableDbError(error: Error): boolean {
    ...
    if (message.includes("disk i/o error") || message.includes("disk full")) {
      return true;
    }
