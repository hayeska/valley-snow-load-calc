...
export class ResilienceLogger {
  ...
  constructor(logDir?: string) {
    const baseLogDir = logDir || path.join(os.homedir(), 'AppData', 'Local', 'ValleySnowLoadCalc', 'logs');
  ...
  private setupGlobalErrorHandlers(): void {
    ...
    process.on('uncaughtException', async (error: Error) => {
      ...
      try {
        ...
        console.log('📝 Critical error logged and emergency recovery attempted');
      } catch (loggingError) {
  ...
  private async logCriticalError(error: Error, operation: string): Promise<void> {
  ...
  private async logUnhandledRejection(error: Error, promise: Promise<any>): Promise<void> {
  ...
  logError(error: Error, context: ErrorContext, recoverable: boolean = true): void {
    const errorInfo = {
      message: error.message,
      name: error.name,
      stack: error.stack,
  ...
  logPerformance(operation: string, duration: number, success: boolean = true, metadata?: Record<string, any>): void {
  ...
  logRecoveryAction(action: string, success: boolean, details?: Record<string, any>): void {
  ...
  logCheckpoint(checkpointId: string, dataSize: number, operation: string): void {
  ...
  logDatabaseOperation(operation: string, success: boolean, duration: number, metadata?: Record<string, any>): void {
  ...
  getErrorSummary(): Record<string, any> {
    ...
    return {
      ...
      mostCommonError: errorArray.length > 0 ?
        errorArray.reduce((prev, current) => current[1] > prev[1] ? current : prev) : null
    };
  }
...
export function logError(error: Error, context: ErrorContext, recoverable: boolean = true): void {
...
export function logRecoveryAction(action: string, success: boolean, details?: Record<string, any>): void {
