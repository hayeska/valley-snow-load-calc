...
async function demonstrateResilientArchitecture(): Promise<void> {
  console.log(
    "🚀 Starting Valley Snow Load Calculator - Comprehensive Error Handling Demo",
  );
  ...
  try {
    ...
    try {
      ...
      if (recoveredData) {
        ...
        console.log(
          `   Last updated: ${recoveredData.updatedAt.toLocaleString()}`,
        );
        console.log(
          "   💡 In a real application, you would be prompted to restore this data\n",
        );
      } else {
      ...
      console.log(
        "⚠️  Crash recovery check failed, continuing with normal startup\n",
      );
    }
    ...
    try {
      ...
      calculator = await withErrorBoundary(
        ...
        { operation: "demo_calculator_creation" },
        {
          Error: async (error: Error) => {
            console.error(
              "❌ Calculator creation failed, attempting recovery...",
            );
      ...
      await withErrorBoundary(
        ...
        { operation: "demo_checkpoint_init" },
        {
          Error: async (error: Error) => {
            console.log(
              "⚠️  Checkpoint system failed, continuing in degraded mode...",
            );
      ...
      console.log(
        "In a real application, this would trigger recovery procedures",
      );
    ...
    const projectWithGeometry = await calculator.updateGeometry(
      projectId,
      geometry,
    );
    console.log(
      "✅ Geometry updated with automatic validation and checkpointing\n",
    );
    ...
    console.log(
      `   Balanced Loads: N=${projectWithInputs.results.balancedLoads.northRoof.toFixed(1)} psf, W=${projectWithInputs.results.balancedLoads.westRoof.toFixed(1)} psf`,
    );
    console.log(
      `   Unbalanced Loads: N=${projectWithInputs.results.unbalancedLoads.northRoof.toFixed(1)} psf, W=${projectWithInputs.results.unbalancedLoads.westRoof.toFixed(1)} psf`,
    );
    console.log(
      `   Valley Loads: H=${projectWithInputs.results.valleyLoads.horizontalLoad.toFixed(1)} psf, V=${projectWithInputs.results.valleyLoads.verticalLoad.toFixed(1)} psf`,
    );
    console.log();
    ...
    console.log(
      `✅ Found ${recoveryOptions.length} recovery options available`,
    );
    ...
    console.log(
      `   Data integrity verified: ${loadedProject.checksum ? "Yes" : "No"}`,
    );
    console.log(
      `   Last updated: ${loadedProject.updatedAt.toLocaleString()}\n`,
    );
    ...
    if (checkpointId !== "checkpoint_failed") {
      const recoveredData = await withErrorBoundary(
        ...
        { operation: "demo_checkpoint_recovery", checkpointId },
        {
          Error: async (error: Error) => {
            console.log(
              "⚠️  Checkpoint recovery failed, attempting alternative recovery...",
            );
      ...
      console.log(
        `✅ Data recovered from checkpoint: ${recoveredData ? recoveredData.name : "Failed"}\n`,
      );
    } else {
      console.log(
        "⏭️  Skipping checkpoint recovery test due to creation failure\n",
      );
    }
    ...
    const testPromises = Array(5)
      .fill(null)
      .map(async (_, i) => {
        try {
          const testProject = await calculator.createProject(
            `Concurrent Test ${i}`,
          );
          ...
          console.log(
            `   ⚠️  Concurrent operation ${i} failed: ${(error as Error).message}`,
          );
          return false;
        }
      });
    ...
    console.log(
      `   ✅ ${successful}/${results.length} concurrent operations succeeded`,
    );
    ...
    allProjects.forEach((project, index) => {
      console.log(
        `   ${index + 1}. ${project.name} (${project.id.substring(0, 8)}...)`,
      );
    });
    console.log();
    ...
    for (const scenario of errorScenarios) {
      ...
      try {
        ...
        console.log(
          `   ✅ ${scenario.name} logged: ${(error as Error).message}`,
        );
      }
    }

    console.log(
      "   📄 All errors have been logged to errors.log with full stack traces\n",
    );
    ...
    console.log(
      `   🏥 System Health: ${health.databaseStatus.toUpperCase()}, ${health.totalProjects} projects`,
    );
    console.log();
    ...
    for (const [operation, stats] of Object.entries(perfStats)) {
      if (stats.count > 0) {
        console.log(
          `   ${operation}: ${stats.count} calls, avg ${(stats.avgDuration * 1000).toFixed(0)}ms, ${stats.successRate * 100}% success`,
        );
      }
    }
    console.log();
    ...
    if (errorSummary.errorPatterns) {
      ...
      Object.entries(errorSummary.errorPatterns)
        .slice(0, 5)
        .forEach(([error, count]) => {
    ...
    console.log(
      "• ✅ Winston logging with error tracking and performance monitoring",
    );
