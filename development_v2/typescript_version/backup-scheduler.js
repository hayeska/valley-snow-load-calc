...
class BackupScheduler {
  constructor(options = {}) {
    this.projectDir =
      options.projectDir || path.join(__dirname, "..", "..", "..");
    this.backupDir =
      options.backupDir ||
      path.join(os.homedir(), "backups", "valley_snow_calc");
    ...
    console.log(
      `☁️  Cloud upload: ${this.config.cloudUpload ? "Enabled" : "Disabled"}`,
    );
  }
  ...
  async ensureBackupFolder() {
    try {
      ...
      if (response.data.files.length > 0) {
        ...
        console.log(
          `📁 Using existing Google Drive folder: ${this.config.googleDriveFolder}`,
        );
      } else {
        // Create new folder
        ...
        console.log(
          `📁 Created Google Drive folder: ${this.config.googleDriveFolder}`,
        );
      }
    } catch (error) {
      console.error(
        "❌ Failed to create/find Google Drive folder:",
        error.message,
      );
    }
  }
  ...
  async cleanupOldBackups() {
    try {
      const backupFiles = fs
        ...
        .filter(
          (file) =>
            file.startsWith("valley_snow_calc_backup_") &&
            file.endsWith(".zip"),
        )
        .map((file) => ({
          name: file,
...
async function main() {
  ...
  if (
    ...
    args.indexOf("--project-dir") + 1 < args.length
  ) {
    options.projectDir = args[args.indexOf("--project-dir") + 1];
  }
  if (
    args.includes("--backup-dir") &&
    args.indexOf("--backup-dir") + 1 < args.length
  ) {
    options.backupDir = args[args.indexOf("--backup-dir") + 1];
  }
  if (
    args.includes("--max-backups") &&
    args.indexOf("--max-backups") + 1 < args.length
  ) {
