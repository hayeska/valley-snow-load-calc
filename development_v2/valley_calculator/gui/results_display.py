...
class ResultsDisplay(ttk.Frame):
    ...
    def _create_summary_tab(self):
        ...
        scrollbar = ttk.Scrollbar(frame, orient="vertical", command=self.summary_text.yview)
    ...
    def _create_diagrams_tab(self):
        ...
        scrollbar = ttk.Scrollbar(frame, orient="vertical", command=self.diagrams_canvas.yview)
        self.diagrams_frame = ttk.Frame(self.diagrams_canvas)

        self.diagrams_frame.bind(
            "<Configure>",
            lambda e: self.diagrams_canvas.configure(scrollregion=self.diagrams_canvas.bbox("all"))
        )

        self.diagrams_canvas.create_window((0, 0), window=self.diagrams_frame, anchor="nw")
    ...
    def _create_detailed_tab(self):
        ...
        scrollbar = ttk.Scrollbar(frame, orient="vertical", command=self.detailed_text.yview)
    ...
    def _create_report_tab(self):
        ...
        ttk.Button(controls_frame, text="Generate PDF Report",
                  command=self._generate_pdf_report).pack(side=tk.LEFT, padx=5)
        ttk.Button(controls_frame, text="Generate HTML Report",
                  command=self._generate_html_report).pack(side=tk.LEFT, padx=5)

        # Report preview
        self.report_text = tk.Text(frame, wrap=tk.WORD, font=("Arial", 10))
        scrollbar = ttk.Scrollbar(frame, orient="vertical", command=self.report_text.yview)
    ...
    def _update_summary_tab(self, results: Dict):
        ...
        self.summary_text.insert(tk.END, f"Project: {inputs.get('project_name', 'Unnamed')}\n")
        self.summary_text.insert(tk.END, f"Location: {inputs.get('location', 'Unknown')}\n\n")

        # Snow loads
        snow_loads = results.get('snow_loads', {})
        self.summary_text.insert(tk.END, "SNOW LOADS:\n")
        self.summary_text.insert(tk.END, f"Ground Snow Load (pg): {snow_loads.get('pg', 0):.1f} psf\n")
        self.summary_text.insert(tk.END, f"Balanced Snow Load (ps): {snow_loads.get('ps_balanced', 0):.1f} psf\n\n")

        # Drift results
        drifts = snow_loads.get('drift_loads', {})
        governing = drifts.get('governing_drift', {})
        self.summary_text.insert(tk.END, "DRIFT ANALYSIS:\n")
        self.summary_text.insert(tk.END, f"Governing Drift Load: {governing.get('pd_max_psf', 0):.1f} psf\n")
        self.summary_text.insert(tk.END, f"Drift Height: {governing.get('hd_ft', 0):.2f} ft\n")
        self.summary_text.insert(tk.END, f"Drift Width: {governing.get('width_ft', 0):.1f} ft\n\n")

        # Slope analysis
        slope_params = results.get('slope_parameters', {})
        self.summary_text.insert(tk.END, "SLOPE PARAMETERS:\n")
        self.summary_text.insert(tk.END, f"North Roof: {slope_params.get('theta_n', 0):.1f}° (S_n = {slope_params.get('S_n', 0):.2f})\n")
        self.summary_text.insert(tk.END, f"West Roof: {slope_params.get('theta_w', 0):.1f}° (S_w = {slope_params.get('S_w', 0):.2f})\n\n")
        ...
        self.summary_text.insert(tk.END, f"North Roof: {'APPLIES' if unbalanced.get('north_applies', False) else 'NOT REQUIRED'}\n")
        self.summary_text.insert(tk.END, f"West Roof: {'APPLIES' if unbalanced.get('west_applies', False) else 'NOT REQUIRED'}\n\n")
        ...
        if beam_analysis:
            ...
            self.summary_text.insert(tk.END, f"Maximum Moment: {max_values.get('moment_lbft', 0):.0f} lb-ft\n")
            self.summary_text.insert(tk.END, f"Maximum Shear: {max_values.get('shear_lb', 0):.0f} lb\n")
            self.summary_text.insert(tk.END, f"Maximum Deflection: {max_values.get('deflection_in', 0):.3f} in\n")
            ...
            self.summary_text.insert(tk.END, f"Bending Check: {'PASS' if bending.get('passes_bending', False) else 'FAIL'} ")
            self.summary_text.insert(tk.END, f"({bending.get('utilization_ratio', 0):.2f})\n")
            self.summary_text.insert(tk.END, f"Shear Check: {'PASS' if shear.get('passes_shear', False) else 'FAIL'} ")
            self.summary_text.insert(tk.END, f"({shear.get('utilization_ratio', 0):.2f})\n")
            self.summary_text.insert(tk.END, f"Deflection Check: {'PASS' if deflection.get('passes_deflection', False) else 'FAIL'} ")
            self.summary_text.insert(tk.END, f"({deflection.get('utilization_ratio', 0):.2f})\n")

            overall_pass = beam_results.get('overall_passes', False)
            self.summary_text.insert(tk.END, f"Overall Result: {'PASS' if overall_pass else 'FAIL'}\n\n")
    ...
    def _create_engineering_diagrams(self, results: Dict):
        ...
        try:
            ...
            fig_plan = self._draw_plan_view(north_span, south_span, ew_half_width, valley_offset)
            ...
            fig_north_drift = self._draw_north_drift_overlay(
                ...
                result_north['hd_ft'], result_north['drift_width_ft'],
                ps_balanced, result_north['pd_max_psf']
            )
            ...
            try:
                fig_snow_dist = self._draw_snow_load_distribution(
                    north_span, south_span, ps_balanced,
                    north_drift.get('pd_max_psf', 0), south_span
                )
            ...
            ax.text(0.5, 0.5, f'Diagram Error:\n{str(e)}',
    ...
    def _draw_north_drift_overlay(self, north_span, south_span, ew_half_width, valley_offset, hd_north, w_north, ps, pd_north):
    ...
    def _draw_drift_load_profile(self, hd: float, w: float, pd_max: float):
        ...
        if hd > 0 and w > 0 and pd_max > 0:
            ...
            # Add annotations
            ax.text(w/2, pd_max/2, f'pd_max = {pd_max:.0f} psf\nhd = {hd:.1f} ft\nw = {w:.1f} ft',
    ...
    def _draw_snow_load_distribution(self, north_span: float, south_span: float, ps_balanced: float,
                                   drift_load: float, drift_width: float):
        ...
        ax.fill_between([0, total_height], ps_balanced, ps_balanced,
                       color='lightgray', alpha=0.5, label='Balanced Snow Load')
        ...
        if drift_load > 0 and drift_width > 0:
            ...
            ax.fill_between([drift_start, south_span], ps_balanced + drift_load, ps_balanced + drift_load,
                           color='lightblue', alpha=0.7, hatch='///', label='Drift Load')

            # Total load annotation
            total_load = ps_balanced + drift_load
            ax.text((drift_start + south_span)/2, ps_balanced + drift_load/2,
                   f'Total: {total_load:.1f} psf', ha='center', va='center',
                   fontsize=10, fontweight='bold', color='navy')

        # Annotations
        ax.text(total_height/2, ps_balanced/2, f'ps = {ps_balanced:.1f} psf',
               ha='center', va='center', fontsize=10, fontweight='bold')

        if drift_load > 0:
            ax.text(south_span * 0.8, ps_balanced + drift_load + 5,
                   f'Drift surcharge: {drift_load:.0f} psf', ha='center', va='bottom',
                   fontsize=9, fontweight='bold', color='blue')
        else:
            ax.text(total_height/2, ps_balanced + 10,
        ...
        ax.text(south_span/2, ps_balanced + 15, 'South Roof', ha='center', fontsize=9, color='red')
        ax.text((south_span + total_height)/2, ps_balanced + 15, 'North Roof', ha='center', fontsize=9, color='red')
    ...
    def _embed_diagram(self, fig, title: str):
        ...
        try:
            ...
            import tkinter as tk
            label = tk.Label(self.diagrams_frame, text=f"Diagram Error: {title}\n{str(e)}",
                           bg='lightcoral', fg='red', font=('Arial', 10))
            label.pack(pady=10)
    ...
    def _generate_text_report(self, results: Dict) -> str:
        ...
        report_lines.append("-" * 30)
        report_lines.append(f"Balanced Snow Load (ps): {snow_loads.get('ps_balanced', 0):.1f} psf")
        report_lines.append("")
        ...
        report_lines.append("")
        report_lines.append("Section 7.6.1 - Unbalanced Snow Loads for Hip and Gable Roofs")
        ...
        report_lines.append("Final design must be performed by a licensed professional engineer.")
        report_lines.append("Verify all loading conditions and local code requirements.")
        report_lines.append("")
