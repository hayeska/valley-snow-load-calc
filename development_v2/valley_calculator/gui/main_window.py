...
from ..core.recovery.error_handlers import error_boundary, resilient_operation
...
class MainWindow:
    ...
    def _check_crash_recovery(self):
        ...
        try:
            ...
            if health_status.get('database_status') == 'error':
                self._show_recovery_dialog("Database Error",
            ...
            for project in recent_projects:
                if project.get('type') == 'database':
                    ...
                    if project_id:
                        recovery_options = self.project_manager.get_project_recovery_options(project_id)
                        if len(recovery_options) > 1:  # More than just basic recovery
                            recovery_needed = True
                            break

            if recovery_needed:
                self._show_recovery_dialog("Recovery Options Available",
    ...
    def _show_recovery_window(self):
        ...
        ttk.Label(recovery_win, text="Available Recovery Options:",
        ...
        try:
            ...
            for project in recent_projects[:5]:  # Show last 5 projects
                if project.get('type') == 'database':
                    ...
                    # Project frame
                    proj_frame = ttk.LabelFrame(options_frame, text=f"Project: {project_name}")
                    proj_frame.pack(fill='x', pady=5)

                    recovery_opts = self.project_manager.get_project_recovery_options(project_id)
                    ...
                    for opt in recovery_opts:
                        ...
                        ttk.Button(proj_frame, text=btn_text,
                                 command=lambda pid=project_id, oid=opt.get('id'):
                                 self._perform_recovery(pid, oid, recovery_win)).pack(fill='x', pady=2)
        ...
        # Close button
        ttk.Button(recovery_win, text="Close", command=recovery_win.destroy).pack(pady=10)

    def _perform_recovery(self, project_id: str, recovery_id: str, parent_window):
        """Perform data recovery operation."""
        try:
            if recovery_id:
                recovered_data = self.project_manager.recover_project_from_checkpoint(recovery_id)
                if recovered_data:
                    ...
                    messagebox.showerror("Recovery Failed",
                                       "Failed to recover project data.")
            else:
                ...
                if project_data:
                    ...
                    self._update_ui_from_project_data()
                    messagebox.showinfo("Recovery Successful",
                                      "Project loaded from last good state.")
                    parent_window.destroy()

        except Exception as e:
            self.logger.log_error(e, operation="perform_recovery",
    ...
    def _start_auto_save_timer(self):
        ...
        def auto_save_worker():
            while not hasattr(self, 'stop_calculation') or not self.stop_calculation.wait(300):  # Auto-save every 5 minutes
                try:
    ...
    def _perform_auto_save(self):
        ...
        if self.current_project and self.current_project_id:
            try:
                ...
                if current_data:
                    self.project_manager.save_project(current_data, self.current_project_id)
                    self.last_auto_save = time.time()

                    # Update status bar
                    self._update_status_bar(f"Auto-saved at {time.strftime('%H:%M:%S')}")

            except Exception as e:
                self.logger.log_error(e, operation="auto_save",
    ...
    def _update_ui_from_project_data(self):
        ...
        try:
            if self.current_project:
                ...
                # Update window title
                project_name = self.current_project.get('project_name', 'Unnamed Project')
    ...
    def _get_current_project_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'project_name': self.current_project.get('project_name', 'Unnamed Project') if self.current_project else 'Unnamed Project',
                'inputs': inputs,
                'results': results,
                'timestamp': time.time()
            }
    ...
    def _create_menu_bar(self):
        ...
        file_menu.add_separator()
        file_menu.add_command(label="Exit", command=self._exit_application, accelerator="Alt+F4")
        ...
        tools_menu.add_command(label="Data Recovery...", command=self._show_recovery_window)
        tools_menu.add_command(label="System Health Check", command=self._show_health_check)
        ...
        view_menu.add_command(label="Toggle Results Panel", command=self._toggle_results_panel)

        # Help menu
        help_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Help", menu=help_menu)
        help_menu.add_command(label="ASCE 7-22 Reference", command=self._show_asce_reference)
    ...
    def _create_main_layout(self):
        ...
        # Input panel
        self.input_panel = InputPanel(left_frame, self.calculator.defaults, self.tooltip_manager)
        ...
        self.calc_button = ttk.Button(button_frame, text="Calculate Snow Loads",
                                    command=self._perform_calculation, style="Accent.TButton")
    ...
    def _export_results(self):
        ...
        if not self.calculation_results:
            messagebox.showwarning("Export Warning", "No results to export. Please calculate first.")
            return
        ...
        if file_path:
            try:
                ...
                messagebox.showinfo("Export Complete", f"Results exported to {exported_path}")
            except Exception as e:
    ...
    def _confirm_discard_changes(self) -> bool:
        ...
        if self.calculation_results:
            result = messagebox.askyesnocancel("Discard Changes",
    ...
    def _change_theme(self, theme_name: str):
        ...
        try:
            ...
            self.status_bar.config(text=f"Theme changed to {self.theme_manager.get_theme_name(theme_name)}")
        except Exception as e:
    ...
    def _reset_defaults(self):
        ...
        if messagebox.askyesno("Reset to Defaults", "Reset all inputs to default values?"):
            self._initialize_defaults()
            self.calculation_results = None
            self.results_display.clear()
    ...
    def _load_template(self):
        ...
        template_name = templates[0] if len(templates) == 1 else templates[0]  # Default to first
...
© 2024 Valley Snow Load Calculator Development Team
    ...
    def _show_health_check(self):
        ...
        try:
            ...
            ttk.Label(health_win, text="System Health Status",
            ...
            status_color = 'green' if db_status == 'healthy' else 'red' if db_status == 'error' else 'orange'

            ttk.Label(status_frame, text=f"Database Status: {db_status.upper()}",
                     foreground=status_color, font=('Arial', 10, 'bold')).pack(anchor='w')

            ttk.Label(status_frame, text=f"Total Projects: {health_status.get('total_projects', 0)}").pack(anchor='w')

            # Error summary
            error_summary = health_status.get('error_summary', {})
            if error_summary.get('total_errors', 0) > 0:
                ttk.Label(status_frame, text=f"Recent Errors: {error_summary['total_errors']}",
                         foreground='red').pack(anchor='w')

                if error_summary.get('most_common_error'):
                    error_type, count = error_summary['most_common_error']
                    ttk.Label(status_frame, text=f"Most Common: {error_type} ({count} times)").pack(anchor='w')

            # Recovery status
            recovery_status = "Ready" if health_status.get('recovery_ready', False) else "Not Ready"
            ttk.Label(status_frame, text=f"Recovery Status: {recovery_status}").pack(anchor='w', pady=(10,0))
            ...
            if db_status != 'healthy':
                ttk.Button(button_frame, text="Run Recovery",
                          command=lambda: self._run_health_recovery(health_win)).pack(side='left', padx=5)

            ttk.Button(button_frame, text="Close",
                      command=health_win.destroy).pack(side='right', padx=5)

        except Exception as e:
            self.logger.log_error(e, operation="show_health_check")
            messagebox.showerror("Health Check Error", f"Failed to check system health: {e}")

    def _run_health_recovery(self, parent_window):
        """Run system health recovery."""
        try:
            # This would implement various recovery strategies
            messagebox.showinfo("Recovery", "System recovery completed. Please restart the application.")
            parent_window.destroy()
        except Exception as e:
