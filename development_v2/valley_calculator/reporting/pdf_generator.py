...
class PDFReportGenerator:
    ...
    def generate_report(self, results: Dict, diagram_figures: list = None, filename: str = None) -> bool:
        """
    ...
    def _create_title_page(self, results: Dict) -> list:
        ...
        # Main title
        title = Paragraph("Valley Snow Load Analysis Report", self.styles['CustomTitle'])
        elements.append(title)
        elements.append(Spacer(1, 0.5*inch))

        # Subtitle
        subtitle = Paragraph("ASCE 7-22 Compliant Engineering Analysis", self.styles['Heading3'])
    ...
    def _create_drift_section(self, results: Dict) -> list:
        ...
        drift_data = [
            ...
            ["North Roof Drift Load", f"{drift_loads.get('north_drift', {}).get('pd_max_psf', 0):.1f} psf"],
            ["West Roof Drift Load", f"{drift_loads.get('west_drift', {}).get('pd_max_psf', 0):.1f} psf"],
        ]
    ...
    def _create_beam_analysis_section(self, results: Dict) -> list:
        ...
        beam_data = [
            ...
            ["Bending Stress Check", f"{'PASS' if stress_checks.get('bending', {}).get('passes_bending', False) else 'FAIL'}"],
            ["Shear Stress Check", f"{'PASS' if stress_checks.get('shear', {}).get('passes_shear', False) else 'FAIL'}"],
            ["Deflection Check", f"{'PASS' if stress_checks.get('deflection', {}).get('passes_deflection', False) else 'FAIL'}"],
            ["Overall Result", "PASS" if beam_results.get('overall_passes', False) else "FAIL"],
        ]
