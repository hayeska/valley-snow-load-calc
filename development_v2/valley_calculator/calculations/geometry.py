...
class RoofGeometry:
    ...
    def calculate_valley_parameters(self, south_span: float, valley_offset: float) -> Dict[str, float]:
        """
    ...
    def calculate_jack_rafter_positions(self, south_span: float, valley_offset: float,
                                      spacing_inches: float = 16.0) -> List[Tuple[float, float]]:
        """
    ...
    def calculate_tributary_areas(self, jack_positions: List[Tuple[float, float]],
                                ew_half_width: float, north_span: float,
                                south_span: float) -> List[Dict[str, float]]:
        """
        ...
        for i, (sloped_dist, horiz_offset) in enumerate(jack_positions):
            ...
            if i == 0:
                ...
                tributary_width = sloped_dist if len(jack_positions) > 1 else sloped_dist * 2
    ...
    def validate_geometry(self, north_span: float, south_span: float,
                         ew_half_width: float, valley_offset: float) -> Tuple[bool, List[str]]:
        """
        ...
        if valley_offset > 0:
            ...
            if valley_angle < self.min_valley_angle:
                errors.append(f"Valley angle ({valley_angle:.1f}°) is unusually shallow")
            if valley_angle > self.max_valley_angle:
                errors.append(f"Valley angle ({valley_angle:.1f}°) is unusually steep")

        return len(errors) == 0, errors

    def calculate_roof_plan_dimensions(self, north_span: float, south_span: float,
                                     ew_half_width: float) -> Dict[str, float]:
        """
